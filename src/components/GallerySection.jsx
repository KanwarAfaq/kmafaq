  ...
  return (
    <div ref={ref} className="relative h-[42vh] min-h-[280px] overflow-hidden flex items-center justify-center">
      <motion.div style={{ y }} className="absolute inset-0 scale-110">
        <div className="w-full h-full bg-cover bg-center" style={{ backgroundImage: `url("${imageUrl || '/afaq-profile.jpeg'}")` }} />
      </motion.div>
      ...
      <div className="relative z-10 text-center px-4 select-none">
        <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-amber-400/80 text-xs font-bold tracking-[0.45em] uppercase mb-3">
          K.M. AFAQ · Research & Professional
        </motion.p>
        <motion.h1 initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-5xl md:text-7xl font-black text-white leading-none" style={{ fontFamily: "'Georgia', serif", letterSpacing: '-0.02em' }}>
          VISUAL <span className="text-amber-400">GALLERY</span>
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-gray-400 text-sm mt-4 tracking-[0.25em] uppercase">
          Research · Academic · Professional Moments
        </motion.p>
...
function FilterBar({ active, setActive, counts, categories }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {all.map(f => (
        <button key={f} type="button" aria-pressed={active === f} onClick={() => setActive(f)} className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-full text-xs font-bold tracking-wider uppercase transition-all duration-300 ${active === f ? 'bg-amber-400 text-black shadow-lg shadow-amber-400/25' : 'bg-white/5 text-gray-400 border border-white/10 hover:border-amber-400/40 hover:text-amber-300'}`}>
          {f}
...
function BentoCard({ img, index, onClick }) {
  ...
  return (
    <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.93 }} transition={{ duration: 0.4, delay: index * 0.045 }} onClick={() => onClick(index)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onClick(index) } }} role="button" tabIndex={0} aria-label={`Open ${img.name || `gallery image ${index + 1}`}`} className={`${getBento(index)} relative group cursor-pointer overflow-hidden rounded-xl bg-gray-900 border border-white/5 hover:border-amber-400/30 transition-colors duration-300`}>
      {!loaded && <div className="absolute inset-0 bg-gradient-to-br from-gray-800 to-gray-900 animate-pulse" />}
      <img src={img.url} alt={img.name || `Gallery image ${index + 1}`} loading="lazy" decoding="async" onLoad={() => setLoaded(true)} onError={() => setLoaded(true)} className={`w-full h-full object-cover transition-all duration-700 group-hover:scale-110 group-hover:brightness-[0.65] ${loaded ? 'opacity-100' : 'opacity-0'}`} />
...
function FilmStrip({ images, onOpen }) {
  ...
  return (
    <div className="relative">
      <button type="button" aria-label="Scroll gallery filmstrip left" onClick={() => scroll(-1)} className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-black/80 border border-white/10 hover:border-amber-400/50 flex items-center justify-center text-gray-400 hover:text-amber-400 transition-all"><FiChevronLeft size={16} /></button>
      <div ref={ref} className="flex gap-3 overflow-x-auto px-10 py-3" style={{ scrollbarWidth: 'none' }}>
        {images.map((img, i) => (
          <div key={img.id || i} role="button" tabIndex={0} aria-label={`Open ${img.name || `gallery image ${i + 1}`}`} onClick={() => onOpen(i)} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onOpen(i) } }} className="relative flex-shrink-0 w-52 h-36 rounded-lg overflow-hidden cursor-pointer group border border-white/5 hover:border-amber-400/40 transition-all">
            <img src={img.url} alt={img.name || `Gallery image ${i + 1}`} loading="lazy" decoding="async" className="w-full h-full object-cover group-hover:scale-110 group-hover:brightness-75 transition-all duration-500" />
      ...
      <button type="button" aria-label="Scroll gallery filmstrip right" onClick={() => scroll(1)} className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-9 h-9 rounded-full bg-black/80 border border-white/10 hover:border-amber-400/50 flex items-center justify-center text-gray-400 hover:text-amber-400 transition-all"><FiChevronRight size={16} /></button>
    </div>
  )
}
...
function Lightbox({ images, index, onClose, onNext, onPrev }) {
  ...
  const handleShare = async () => {
    try {
      if (navigator.share) await navigator.share({ title: img.name, url: img.url })
      else if (navigator.clipboard) await navigator.clipboard.writeText(img.url)
    } catch (error) {
      if (error?.name !== 'AbortError') console.error('Unable to share gallery image:', error)
    }
  }

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} role="dialog" aria-modal="true" aria-label={img.name || 'Gallery image viewer'} className="fixed inset-0 z-[9999] bg-black/96 backdrop-blur-md flex items-center justify-center" onClick={onClose}>
      ...
      <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-5 py-4 z-10 bg-gradient-to-b from-black/80 to-transparent" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-center gap-2">
          ...
          <button type="button" aria-label="Share image" onClick={handleShare} className="w-9 h-9 rounded-full bg-white/10 hover:bg-amber-400/20 border border-white/10 hover:border-amber-400/40 flex items-center justify-center text-gray-300 hover:text-amber-400 transition-all"><FiShare2 size={14} /></button>
          <button type="button" aria-label="Download image" onClick={handleDownload} className="w-9 h-9 rounded-full bg-white/10 hover:bg-amber-400/20 border border-white/10 hover:border-amber-400/40 flex items-center justify-center text-gray-300 hover:text-amber-400 transition-all"><FiDownload size={14} /></button>
          <button type="button" aria-label="Close image viewer" onClick={onClose} className="w-9 h-9 rounded-full bg-white/10 hover:bg-red-500/30 border border-white/10 hover:border-red-500/40 flex items-center justify-center text-gray-300 hover:text-red-400 transition-all"><FiX size={15} /></button>
        </div>
      </div>
      <button type="button" aria-label="Previous image" onClick={e => { e.stopPropagation(); onPrev() }} className="absolute left-3 md:left-5 z-10 w-11 h-11 rounded-full bg-black/60 hover:bg-amber-400/20 border border-white/10 hover:border-amber-400/50 flex items-center justify-center text-gray-300 hover:text-amber-400 transition-all backdrop-blur-sm"><FiChevronLeft size={22} /></button>
      <AnimatePresence mode="wait"><motion.img key={img.id || index} src={img.url} alt={img.name || 'Gallery image'} decoding="async" initial={{ opacity: 0, scale: 0.93 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 1.05 }} transition={{ duration: 0.32 }} onClick={e => e.stopPropagation()} className="max-w-[88vw] max-h-[80vh] object-contain rounded-xl shadow-2xl shadow-black border border-white/5" /></AnimatePresence>
      <button type="button" aria-label="Next image" onClick={e => { e.stopPropagation(); onNext() }} className="absolute right-3 md:right-5 z-10 w-11 h-11 rounded-full bg-black/60 hover:bg-amber-400/20 border border-white/10 hover:border-amber-400/50 flex items-center justify-center text-gray-300 hover:text-amber-400 transition-all backdrop-blur-sm"><FiChevronRight size={22} /></button>
    </motion.div>
  )
}
...
export default function GallerySection() {
    ...
    const fetchGallery = async () => {
      ...
      if (error) {
        ...
        const ordered = [...(data || [])].sort((a, b) => {
          const aOrder = a.display_order == null || a.display_order === '' ? Number.MAX_SAFE_INTEGER : Number(a.display_order);
          const bOrder = b.display_order == null || b.display_order === '' ? Number.MAX_SAFE_INTEGER : Number(b.display_order);
  ...
  return (
    <section id="gallery" className="bg-black text-white relative py-12">
      <HeroBanner imageUrl={dbImages[0]?.url} />
      ...
      <div className="sticky top-16 z-30 bg-black/90 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 bg-white/5 rounded-full p-1 border border-white/10">
            <button type="button" aria-pressed={viewMode === 'bento'} onClick={() => setViewMode('bento')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'bento' ? 'bg-amber-400 text-black' : 'text-gray-500 hover:text-amber-400'}`}><FiGrid size={12} /> Grid</button>
            <button type="button" aria-pressed={viewMode === 'filmstrip'} onClick={() => setViewMode('filmstrip')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider transition-all ${viewMode === 'filmstrip' ? 'bg-amber-400 text-black' : 'text-gray-500 hover:text-amber-400'}`}>▶ Film</button>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        ...
        {loading && <div className="py-20 text-center text-sm text-gray-500">Loading gallery…</div>}
        {!loading && loadError && <div className="py-20 text-center text-sm text-red-300">{loadError}</div>}

        {!loading && !loadError && viewMode === 'filmstrip' && (
        ...
        {!loading && !loadError && viewMode === 'bento' && (
